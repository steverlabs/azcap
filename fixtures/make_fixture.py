    ...
    for fam, (pat, sizes) in FAMILIES.items():
        ...
        for n in sizes:
            ...
            if quota_zero:
                ...
                r.append({"type": "Location", "reason": "NotAvailableForSubscription", "locations": [region], "zones": []})
            elif zones and random.random() < p_zone * (1.5 if n >= 32 else 1):
                blocked = sorted(random.sample(zones, random.choice([1,1,2])))
                r.append({"type": "Zone", "reason": "NotAvailableForSubscription", "locations": [region], "zones": blocked})
            skus.append({"region": region, "name": name, "family": fam,
                         "capabilities": {"vCPUs": str(n), "MemoryGB": str(n * (8 if "E" in pat[9] else 4))},
                         "zones": zones, "restrictions": r})
        limit = 0 if quota_zero else random.choice([100, 200, 350, 500])
        quota.append({"region": region, "family": fam, "localized": fam, "current": 0 if not limit else random.randint(0, limit), "limit": limit})

locations = {r: {"name": r, "display": r, "geography": g, "geography_group": gg, "pair": p,
                 "zonal": bool(REGIONS[r][0])} for r, (g, gg, p) in LOCATIONS.items()}
out = {"meta": {"subscription": "00000000-fixture", "generated": "synthetic"},
       "locations": locations, "skus": skus, "quota": quota}
Path(__file__).with_name("sample.json").write_text(json.dumps(out, indent=1))
